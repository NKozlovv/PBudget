...
- **`trip_details`** — added 2026-09-19 for the Trips page (`/trips`). One row per (budget_id, trip): `travelers` (integer, default 1), plus **`start_date`/`end_date`** (date, nullable, added same day as a follow-up) — an explicit override for the trip's date range. Null until the user sets them via the page's "edit trip" modal; `lib/trips/summary.ts` falls back to a guess (earliest/latest tagged transaction date) until then, since a transaction's date is when money moved, not necessarily a day you were on the trip (a flight bought weeks ahead would otherwise stretch the guess back to the booking date). PK is `(budget_id, trip)` rather than an id, matching `trip`'s own name-not-id identity above.
